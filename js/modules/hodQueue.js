        ...
        <tbody>
          ${passes
            ...
            <tr>
              ...
              <td>
                <div class="space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-200/80 text-xs font-medium">
                  <div class="text-emerald-800 font-semibold">Verified: ${escapeHtml(p.counselorApproval?.counselorName || p.counselorName || 'Counselor')}</div>
                  <div class="text-indigo-800 font-semibold">Endorsed: ${escapeHtml(p.advisorApproval?.advisorName || 'Class Advisor')}</div>
                </div>
              </td>
              <td class="text-right">
...
/**
 * On-Duty (OD) Head of Department (HOD) Queue & Final Authorization
 */
async function fetchHODODQueue() {
  const el = document.getElementById('hodODQueue');
  if (!el || !loggedUser) return;
  ...
  try {
    ...
    if (!odRequests || odRequests.length === 0) {
      el.innerHTML = `
        <div class="p-12 text-center bg-white space-y-3">
          ...
          <p class="text-xs md:text-sm text-slate-500 max-w-sm mx-auto">No On-Duty requests awaiting final authorization for ${loggedUser.dept} Department.</p>
        </div>`;
      return;
    }
            ...
            .map(od => {
              ...
              return `
                <tr>
                  ...
                  <td>
                    <div class="font-mono text-xs space-y-1 bg-slate-50 border border-slate-200 p-2.5 rounded-xl">
                      <div class="text-emerald-800 font-semibold"><span class="font-bold">1. Counsellor:</span> ${escapeHtml(od.counselorApproval?.counselorName || 'Done')}</div>
                      <div class="text-indigo-800 font-semibold"><span class="font-bold">2. Advisor:</span> ${escapeHtml(od.advisorApproval?.advisorName || 'Done')}</div>
                    </div>
                  </td>
                  <td>
                  ...
                  <td class="text-right">
                    <div class="flex items-center justify-end gap-1.5">
                      ...
                      <button onclick="openRejectModal('${od._id}', 'HOD', true)" class="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200/80 font-semibold rounded-xl shadow-2xs whitespace-nowrap transition active:scale-95 text-xs">
                        Reject
                      </button>
                      <button onclick="approveHODOD('${od._id}')" class="px-3 py-1.5 bg-purple-700 hover:bg-purple-800 text-white font-semibold rounded-xl shadow-2xs whitespace-nowrap transition active:scale-95 text-xs flex items-center gap-1">
    ...
    el.innerHTML = `<div class="p-8 text-center text-xs md:text-sm text-rose-500 font-semibold">Failed to load HOD On-Duty queue.</div>`;
  }
}

async function approveHODOD(requestId) {
  try {
    const data = await Api.post('/api/onduty/approve/hod', {
      requestId,
      hodName: loggedUser.name
    });
    if (data && (data.success === false || data.error)) {
    ...
    showToast(data.message || 'On-Duty finalized! Status is now Completed.', 'success', 3000);
    refreshAllAuthorityViews();
  } catch (err) {
    showToast('Server error while finalizing On-Duty.', 'error', 3500);
  }
}

window.fetchHODQueue = fetchHODQueue;
window.approveHODPass = approveHODPass;
window.fetchHODODQueue = fetchHODODQueue;
window.approveHODOD = approveHODOD;
